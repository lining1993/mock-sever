...
public class FileUtils extends org.apache.commons.io.FileUtils {
    ...
    public static boolean copyFileCover(String srcFileName,
        ...
        if (descFile.exists()) {
            ...
            if (!descFile.getParentFile().exists()) {
                ...
                logger.error("目标文件所在的目录不存在，创建目录!");
                // 创建目标文件所在的目录
        ...
        try {
            ...
            logger.error("复制单个文件 " + srcFileName + " 到" + descFileName
                    + "成功!");
            return true;
        } catch (Exception e) {
    ...
    public static boolean copyDirectoryCover(String srcDirName,
        ...
        if (descDir.exists()) {
            if (coverlay) {
                // 允许覆盖目标目录
                logger.error("目标目录已存在，准备删除!");
                if (!FileUtils.delFile(descDirNames)) {
                    logger.error("删除目录 " + descDirNames + " 失败!");
                    return false;
                }
            } else {
                logger.error("目标目录复制失败，目标目录 " + descDirNames + " 已存在!");
                return false;
            }
        } else {
            // 创建目标目录
            logger.error("目标目录不存在，准备创建!");
            if (!descDir.mkdirs()) {
        ...
        if (!flag) {
            logger.error("复制目录 " + srcDirName + " 到 " + descDirName + " 失败!");
            return false;
        }
        logger.error("复制目录 " + srcDirName + " 到 " + descDirName + " 成功!");
        return true;
    ...
    public static boolean deleteDirectory(String dirName) {
        ...
        // 删除当前目录
        if (dirFile.delete()) {
            logger.error("删除目录 " + dirName + " 成功!");
            return true;
        } else {
            logger.error("删除目录 " + dirName + " 失败!");
            return false;
        }
    ...
    public static boolean createFile(String descFileName) {
        ...
        if (file.exists()) {
            logger.error("文件 " + descFileName + " 已存在!");
            return false;
        }
...
//            }
        ...
        try {
            if (file.createNewFile()) {
                logger.error(descFileName + " 文件创建成功!");
                return true;
            } else {
                logger.error(descFileName + " 文件创建失败!");
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static boolean createDirectory(String descDirName) {
        ...
        File descDir = new File(descDirNames);
        if (descDir.exists()) {
            logger.error("目录 " + descDirNames + " 已存在!");
            return false;
        }
        // 创建目录
        if (descDir.mkdirs()) {
            logger.error("目录 " + descDirNames + " 创建成功!");
            return true;
        } else {
            logger.error("目录 " + descDirNames + " 创建失败!");
            return false;
        }
     ...
     * @return 返回错误信息，无错误信息返回null
     */
    public static String downByte(byte[] file, HttpServletRequest request, HttpServletResponse response, String fileName) {
        String error = null;
        if (error != null) {
...
//        RandomAccessFile raf = null; // 负责读取数据
        ...
        if (request.getHeader("Range") != null) { // 客户端请求的下载的文件块的开始字节
            ...
            logger.error("request.getHeader(\"Range\") = " + request.getHeader("Range"));
        ...
        if (pastLength != 0) {
            ...
            logger.error("---------------不是从开始进行下载！服务器即将开始断点续传...");
            switch (rangeSwitch) {
            ...
            // 是从开始下载
            logger.error("---------------是从开始进行下载！");
        }

        try {
            response.addHeader("Content-Disposition", "attachment; filename=\"" +
                    Encodes.urlEncode(fileName) + "\"");
            response.setContentType(getContentType(fileName)); // set the MIME type.
            response.addHeader("Content-Length", String.valueOf(contentLength));
            os = response.getOutputStream();
            out = new BufferedOutputStream(os);
//            raf = new RandomAccessFile(file, "r");
            try (InputStream raf = new ByteArrayInputStream(file);){
                switch (rangeSwitch) {
                    case 0: { // 普通下载，或者从头开始的下载 同1
                    }
                    case 1: { // 针对 bytes=27000- 的请求
                        raf.skip(pastLength); // 形如 bytes=969998336- 的客户端请求，跳过 969998336 个字节
                        int n = 0;
                    ...
                    case 2: { // 针对 bytes=27000-39000 的请求
                        raf.skip(pastLength); // 形如 bytes=1275856879-1275877358 的客户端请求，找到第 1275856879 个字节
                        int n = 0;
...
//            }
    ...
    public static String readFile(String filePath, String encoding) throws IOException {
        ...
        FileUtils.readToBuffer(sb, filePath, encoding);
        return sb.toString();
    }
}
